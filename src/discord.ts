...
export async function sendDiscordNotification(message: string): Promise<void> {
  if (!DISCORD_WEBHOOK_URL) {
    console.warn("⚠️ The Sentinel: Discord Webhook URL is not set.");
    return;
  }

  try {
    await axios.post(DISCORD_WEBHOOK_URL, { content: message });
    console.log("✅ The Sentinel: Sent notification to Discord:", message);
  } catch (error: any) {
    console.error("❌ The Sentinel: Failed to send Discord notification:", error.response?.data || error.message);
  }
}
...
export async function sendTransferSuccessNotification(amount: string, recipient: string, txId?: string): Promise<void> {
  await sendDiscordNotification(
    `✅ **The Sentinel: Transfer Successful!**\n- **Amount:** ${amount} GALA\n- **Recipient:** ${recipient}` + (txId ? `\n- **Transaction ID:** ${txId}` : "")
  );
}
